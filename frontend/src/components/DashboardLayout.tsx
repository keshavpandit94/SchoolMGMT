...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ children }) => {
  ...
  const getNavItems = () => {
    ...
    if (user) {
      ...
      // Teachers (Admin and Principal get management; Teachers and Staff get read-only)
      items.push({ text: 'Teachers', path: '/teachers', icon: <UserCheck size={20} /> });

      // Staff (Admin and Principal get management; Teachers and Staff get read-only)
      items.push({ text: 'Staff Members', path: '/staff', icon: <Contact size={20} /> });

      // Inventory (all roles CRUD)
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#0F172A', color: '#F8FAFC' }}>
      ...
      <Box sx={{ p: 2.5, display: 'flex', alignItems: 'center', gap: 1.5, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        ...
        <Box sx={{ overflow: 'hidden' }}>
          ...
          <Typography variant="caption" sx={{ color: '#6366F1', fontWeight: 'semibold', display: 'block' }}>
            {user?.role}
          </Typography>
        </Box>
      </Box>
